...
**This folder contains the code to check computationally the bound of Theorem 5.4**
...
The code generates checks and computes the difference between the computer-obtained value and the theoretical value for multiple epsilon, m, and M values. The algorithm returns CSV files with the difference. 
...
* `FinalMaximumFunction.py`: Checks and generates the CSV file containing the theoretical and computational results and their difference of the function explained in Remark A.11 (or Remark B.22 in the long version of the paper) for all epsilon between `0.01` and `1.99` (step=`0.01`), between `2` and `9.9` (step=`0.1`), and between `10` and `100` (step=`1`); and for all `(m,M)` with granularity `0.01`.
* `FinalMaximumInverse.py`: Checks and generates the CSV file containing the theoretical and computational results and their difference of the function explained in Remark A.11 (or Remark B.24 in the long version of the paper) for all epsilon previously listed and all `(m,M)` with granularity `0.01`.  
...
`FinalMaximumFunction.py` outputs a CSV file (`output.csv`) containing the computational (`DiffEvol`) and hypothesized theoretical values (`HypValue`), and their difference (`Difference`=`DiffEvol`−`HypValue`). The script also outputs error messages in the terminal when:
...
`FinalMaximumInverse.py` outputs a CSV file (`output.csv`) containing the computational (`DiffEvol`) and hypothesized theoretical values (`HypValue`), and their difference (`Difference`=`DiffEvol`−`HypValue`).  The script also outputs error messages in the terminal when:
* `DiffEvol` is larger than `HypValue` (up to some floating error), which would contradict our claim in the paper.
