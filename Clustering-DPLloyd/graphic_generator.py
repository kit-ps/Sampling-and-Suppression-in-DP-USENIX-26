...
def generate_plot_suppression(plot_path_start, csv_path, plot_values, list_m_and_M, file_name_m_and_M, plots_limits, statistic, epsilon, include_title=True):
    ...
    if(plot_values=="difference_error_M_minus_MoS"):
        title = "DPLloyd clustering: Difference of NICV (" + title_end +")"
...
def generate_plot_uniform_Poisson_sampling(plot_path_start, csv_path_list_M_Average, csv_path_list_M_Variance, csv_path_list_MoSChange_Average, csv_path_list_MoSChange_Variance, epsilon_list, plot_type, numberofrepeat, include_p1=True, include_title=True):
    ...
    for eps, csv_path_M_Average, csv_path_M_Variance, csv_path_MoSChange_Average, csv_path_MoSChange_Variance in zip(epsilon_list, csv_path_list_M_Average, csv_path_list_M_Variance, csv_path_list_MoSChange_Average, csv_path_list_MoSChange_Variance):
        ...
        df_M_plot_Average = df_M_Average.filter(["m","stat_NICV"],axis=1).sort_values(by=["m"])
        df_M_plot_Variance = df_M_Variance.filter(["m","stat_NICV"],axis=1).sort_values(by=["m"])
        df_MoSChange_plot_Average = df_MoSChange_Average.filter(["m","stat_NICV"],axis=1).sort_values(by=["m"])
        df_MoSChange_plot_Variance = df_MoSChange_Variance.filter(["m","stat_NICV"],axis=1).sort_values(by=["m"])
        ...
        #if include_p1==True:
        #    value_p1_Average = df_M_plot_Average["stat_NICV"].iloc[0]
        ...
        #    value_p1_Variance = df_M_plot_Variance["stat_NICV"].iloc[0]
    ...
    if plot_type=="Variance":
        ax.set(xlabel='Sampling rate',ylabel= "Variance of NICV (log scale)")
    else:
        ax.set(xlabel='Sampling rate',ylabel= "Average of NICV (log scale)")

    #Print Title
    title = "DPLloyd clustering mechanism"
    if include_title==True:
        ax.set_title(title)
