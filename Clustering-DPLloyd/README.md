...
* `columns-range`: List of the columns name and their ranges of the database to test. The must be introduced as `name:lower_bound:upper_bound` (separated with `:`), and with spaces in-between columns.
...
The `generateFileandGraph` function generates all the CSV files and plots for the specified database. The inputs of equivalent to the parameters above, where `columns-range` is now separated into the columns name (`columns`) and its range (`range_columns`) as seen in the previous command.  
...
* Files of the type `eps=[epsilon]_[mechanism].csv` containing the normalized intracluster variance (NICV) of DPLloyd for every of the `repetitions` iterations. A missing number appearing in the CSV file means that the mechanism could not be run for the given epsilon parameter. One file is generated for every `epsilon` in `list_epsilons`, and there are four `mechanism` variants:
