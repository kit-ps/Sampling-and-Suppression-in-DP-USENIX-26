...
The time to run is dependent on the size of the database. In our case, the run on the `adult_clustering` database took around 2.5&nbsp;days (60&#8201;h). The code contains a parallelization into 64 pools. 
...
* Files of the type `eps=[epsilon]_[mechanism].csv` containing the normalized intracluster variance (NICV) of DPLloyd for every of the `numberofrepeat` iterations. A missing number appearing in the CSV file means that the mechanism could not be run for the given epsilon parameter. One file is generated for every `epsilon` in `list_epsilons`, and there are four `mechanism` variants:
...
* The plots covering the evaluation for outlier-score suppression (Section 6). Each figure in the paper contains four variants changing the `[epsilon]` value to  `0.25`, `0.5`, `1`, or `2`. Forms part of Appendix A.6 (long version):
	* `eps=[epsilon]_difference_error_M_minus_MoSChangeEpsDelta_Average_10--90.pdf`: Figure 2 (bottom row) and Figure 44 in long version.
...
* `normalize_database.py` contains the function that normalized the database, a requirement for DPLloyd. 
...
The database used in this experiment is the Adult (`adult_clustering.csv`) databases. In addition, in the experiment folder we find these subfolders: 
