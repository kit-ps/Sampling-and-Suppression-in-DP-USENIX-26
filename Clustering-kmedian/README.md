...
The code is parallelized, and the number of processes corresponds to the number of CPU cores on the user's machine. This setting can be modified by changing the value of `N_CORES` in the `config.py` file in the main folder. In our case with 64 cores, the run on the `database1` database took around a day (24&#8201;h). 
...
* Files of the type `eps=[epsilon]_[mechanism].csv` containing the average cost of the $k$-median algorithm run multiple times (total number given by `repetitions`). A missing number appearing in the CSV file means that the mechanism could not be run for the given epsilon parameter. One file is generated for every `epsilon` in `list_epsilons`, and there are four `mechanism` variants:
	* `M`: The $k$-median clustering mechanism (M) without suppression run for the given epsilon.
	* `MoS`: The $k$-median clustering mechanism (M) with suppression (S) run for the given epsilon.
	* `M_ChangeEpsDelta`: The $k$-median clustering mechanism (M) without suppression run for the epsilon that ensures that M and MoS have the same privacy parameters.  
	* `MoS_ChangeEpsDelta`: The $k$-median clustering mechanism (M) with suppression (S) run for the epsilon that ensures that M and MoS have the same privacy parameters.
