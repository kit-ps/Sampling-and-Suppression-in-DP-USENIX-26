...
## Generates a simple synthetic database of integers inside col_dimension 
# A database of integers is generated by sampling from the Gaussian distribution centered on a number of clusters
# Generates too the domain file
# The following variables can be selected:
# * database_file_name: Name of the database and its domain.
# * size_database: Number of records in database
# * col_dimensions: Domain of the integers (each column needs to be named with col_names)
# * number_clusters: Number of clusters
# * variance_normal: Variance of the Gaussian distribution
# * clusters: Predefine the clusters of select "None" so that they are randomly selected inside the domain (ensures that no cluster is given at the edges or very near to the edges)
# * weight_clusters: Allows to weigh generation of records to the clusters. Select "None" for uniform weights.
