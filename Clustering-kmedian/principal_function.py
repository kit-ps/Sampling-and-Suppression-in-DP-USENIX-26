...
def generateFileandGraph(database_name, columns, data_domain_name, main_folder_name, number_clusters, list_epsilons=[0.25,0.5,1,2], repetitions: int = 20):
    ...
    for eps in list_epsilons:
        ...
        MoS_Clustering(output_file_name = file_name_start + "_MoS.csv", df=df, columns=columns, data_domain=data_domain, path_average_distances=file_name_average_distance_list, m_and_M=m_and_M_combined, epsilon=eps, number_clusters=number_clusters, value_n=value_n, EpsDeltaChange=False, pbar=pbar, repetitions=repetitions)
        MoS_Clustering(output_file_name = file_name_start + "_MoS_ChangeEpsDelta.csv", df=df, columns=columns, data_domain=data_domain, m_and_M=m_and_M_combined, path_average_distances=file_name_average_distance_list, epsilon=eps, number_clusters=number_clusters, value_n=value_n, EpsDeltaChange=True, pbar=pbar, repetitions=repetitions)
        M_Clustering(output_file_name = file_name_start + "_M.csv", df=df, columns=columns, m_and_M=m_and_M_combined, data_domain=data_domain, epsilon=eps, number_clusters=number_clusters, value_n=value_n, EpsDeltaChange=False, pbar=pbar, repetitions=repetitions)
        M_Clustering(output_file_name = file_name_start + "_M_ChangeEpsDelta.csv", df=df, columns=columns, m_and_M=m_and_M_combined, data_domain=data_domain, epsilon=eps, number_clusters=number_clusters, value_n=value_n, EpsDeltaChange=True, pbar=pbar, repetitions=repetitions)
